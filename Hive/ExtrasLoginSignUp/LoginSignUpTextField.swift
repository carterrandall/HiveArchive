...
//  Hive
//
//  Created by Carter Randall on 2018-12-13.
//  Copyright © 2018 Carter Randall. All rights reserved.
//

import UIKit

protocol phoneValidatorDelegate {
    func handlePhoneTextChange()
    func handleDisplayCountryCode()
}

class LoginSignUpphoneField: UIView, UITextFieldDelegate {
    
    var phoneDelegate : phoneValidatorDelegate?
    ...
    let phoneField : UITextField = {
        ...
        tf.attributedPlaceholder = NSAttributedString(string: "Mobile Number", attributes: [NSAttributedString.Key.foregroundColor : UIColor(white: 0, alpha: 0.3), .font: UIFont.systemFont(ofSize: 14)])
    ...
    func handleAutoCompletePhone(string: String, textField: UITextField) -> Bool{
        let newstring = string.replacingOccurrences(of: " ", with: "").replacingOccurrences(of: "-", with: "")
        print(newstring.count)
        if (Int(newstring) != nil && newstring.count == 10){
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if (string.count == 13){ // does this work generally, I hope so
            return handleAutoCompletePhone(string: string, textField: textField)
        }else{
    ...
    func format(phoneNumber: String, shouldRemoveLastDigit: Bool = false) -> String {
        ...
        if (number.count < 10){
            if (number.count == 0){
                ...
                validButton.isHidden = false
            }
            isPhoneValid = false
            validButton.setImage(UIImage(named: "badform"), for: .normal)
        } else {
            isPhoneValid = true
            validButton.isHidden = false
            validButton.setImage(UIImage(named: "check"), for: .normal)
        }
        if number.count > 10 {
        ...
        phoneDelegate?.handlePhoneTextChange()
        print(isPhoneValid)
        return number
    }
    ...
    let phoneCodeButon : UIButton = {
        let codebutton = UIButton()
        codebutton.titleLabel?.textAlignment = .center
        codebutton.setTitle("CAN +1", for: .normal)
        codebutton.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        codebutton.setTitleColor(UIColor.mainRed(), for: .normal)
        //        codebutton.setAttributedTitle(NSAttributedString(string: "CAN", attributes: [NSAttributedString.Key.foregroundColor : UIColor(white: 0, alpha: 0.3), .font: UIFont.systemFont(ofSize: 14)]), for: .normal)
        codebutton.addTarget(self, action: #selector(handleCountryCode), for: .touchUpInside)
        return codebutton
    }()
    ...
    override init(frame: CGRect) {
        ...
        addSubview(phoneCodeButon)
        
        phoneCodeButon.anchor(top: topAnchor, left: leftAnchor, bottom: bottomAnchor, right: nil, paddingTop: 0, paddingLeft: 8, paddingBottom: 0, paddingRight: 0, width: frame.width*(3/10), height: frame.height)
        phoneField.anchor(top: topAnchor, left: phoneCodeButon.rightAnchor, bottom: bottomAnchor, right: rightAnchor, paddingTop: 0, paddingLeft: 2, paddingBottom: 0, paddingRight: 0, width: frame.width*(7/10), height: frame.height) // (4/5 = 0.80)
        
        phoneField.delegate = self
        addSubview(validButton)
        validButton.isHidden = true
        validButton.anchor(top: nil, left: nil, bottom: nil, right: phoneField.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 8, width: 16, height: 16)
        validButton.centerYAnchor.constraint(equalTo: centerYAnchor).isActive = true
        tintColor = .black
    ...
    func handleDefaultCountry() {
        if let code = NSLocale.current.regionCode, let index = countryJSON.firstIndex(where: { (dict) -> Bool in
            return code == dict["code"]
        }), let ext = countryJSON[index]["dial_code"] { // also could use the country thing for this
            phoneCodeButon.setTitle(code + " " + ext, for: .normal)
            countryCode = ext.replacingOccurrences(of: "+", with: "")
        }else{
            countryCode = "1"
        }
    }
    ...
    @objc func handleCountryCode() {
        phoneDelegate?.handleDisplayCountryCode()
        // basically need to be able to scroll through the list below, which is already in alphabetical order, and select a country, then we set the variables and button title and dismiss this view
        // countryCode = the one selected.removing occureance of + so it is just a String of pure numbers)
        // phonecodebutton.settitle = country code (the two letters) + " " + the selected one, leaving in the plus this time for aestetics.
...
class LoginSignUpTextField: UITextField {
    ...
    override init(frame: CGRect) {
        ...
        addSubview(validButton)
        validButton.isHidden = true
        validButton.anchor(top: nil, left: nil, bottom: nil, right: rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 8, width: 16, height: 16)
        validButton.centerYAnchor.constraint(equalTo: centerYAnchor).isActive = true
